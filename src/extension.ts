...
// Set to true the first time reloadJavaProjects successfully executes a Java command so that
// startup retry timers stop firing after the Language Server is confirmed ready.
let javaReloadSucceeded = false;
...
scheduleStartupJavaProjectReloadRetries();
}
...
/**
 * Schedules additional java.reloadProjects attempts after activation to ensure the Java Language
 * Server has had time to start before we request a project reload. Each retry is a no-op once the
...
// Java commands are available — mark startup retries as no longer needed.
javaReloadSucceeded = true;
